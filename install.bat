...
if %errorlevel% neq 0 (
    ...
    echo [错误] 依赖安装失败!
    echo 常见原因:
    echo   1. 网络连接问题(可尝试切换网络或使用国内镜像)
    echo   2. requirements.txt 中某个包名拼写错误
    echo   3. 系统缺少 C++ 构建工具(部分包需要编译)
    echo.
    echo 国内用户可尝试使用清华镜像源重新安装:
...
echo.
echo ============================================================
echo   ✓ 环境准备完成,现在可以使用 tender-writer 了!
echo ============================================================
echo.
...
echo.
echo   [方式三] 纯对话 AI(Kimi / 智谱清言 / 豆包 / 文心一言 等):
echo     无需环境准备,直接把 SKILL.md 和 references/ 目录下的文档
echo     上传到 AI 知识库即可。Python 脚本不可执行,但工作流仍可生效。
echo.
