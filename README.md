...
> 一套面向政府类项目投标场景的 AI 协作式技术标编制工具包,覆盖招标文件解析、评分矩阵拆解、章节生成、文档组装与合规核查。

适用于 Claude Code、ChatGPT、Qwen Code、Cline 等能读写文件或进行多轮对话的 AI 工具,目标是把原本依赖个人经验的标书编制过程固化成可复用、可审查、可沉淀的工作流。

## 项目简介

**中文**: `tender-writer` 是一套面向政府类项目和规范化招投标场景的技术标编制工具包。它把招标文件解析、评分项拆解、提纲生成、正文组装和合规检查串成一条稳定工作流,降低个人经验依赖,提升成稿速度与可复核性。

**English**: `tender-writer` is an AI-assisted toolkit for technical proposal writing in government and formal bidding scenarios. It turns tender parsing, scoring decomposition, outline generation, document assembly, and compliance review into a repeatable workflow that is easier to audit, reuse, and scale.

## 快速开始

1. 克隆或下载本仓库到本地。
2. 在仓库根目录双击 `install.bat`,创建 `.venv` 并安装依赖。
3. 根据你使用的 AI 工具选择接入方式:
   - Claude Code: 放到 `.claude\skills\tender-writer\` 下使用。
   - Qwen Code / Trae / Cline: 让 AI 先阅读 `SKILL.md` 再按五阶段推进。
   - 纯对话 AI: 上传 `SKILL.md`、`references/`、`templates/` 作为知识库。
4. 用 `run_script.bat` 调用脚本,不要直接切系统 Python。

```bat
install.bat
run_script.bat parse_tender.py "D:\你的项目\招标文件.pdf"
run_script.bat build_scoring_matrix.py "output\tender_brief.md"
run_script.bat generate_outline.py "output\scoring_matrix.csv"
```

## 典型使用示例

**Claude Code / 本地 AI 提示词示例**

```text
请先读取 tender-writer/SKILL.md,严格按五阶段工作流帮我编制技术标。
所有 Python 脚本通过 tender-writer/run_script.bat 调用。
当前要处理的招标文件是 D:\项目\招标文件.pdf。
先执行阶段 1,输出 tender brief,等我确认后再进入下一阶段。
```

**纯对话 AI 提示词示例**

```text
你已经读过我上传的 SKILL.md、references 和 templates。
请先执行阶段 1: 招标文件解析。
严格按 tender_brief 模板输出,缺失字段标注【待补充】,不要脑补。
等我确认后再进入阶段 2。
```

## 一、这是什么

这是一套**政府类项目技术标编制的工程化方案**——它把"读懂招标文件 → 拆解评分办法 → 反推章节结构 → 分章节撰写 → 合规终审"这一整套标书编制流程,固化成了可复用的脚本、模板、规范和工作流提示词。

它**对 Claude Code 支持最好**(以 SKILL.md 形式自动加载、自动触发、全流程自动化),但同时**也可以在其他 AI 工具上使用**——只要那个 AI 能读懂 markdown 并能跟你做多轮对话。

整套方案是**自包含**的:自带 Python 脚本、自带依赖清单、自带一键安装脚本,不依赖 git、不依赖全局配置、不污染你电脑上的 Python 环境。把整个 `tender-writer/` 文件夹复制走就能用。

---

## 二、能帮你做什么

1. **快速吃透招标文件**:把一份动辄几十上百页的招标文件,自动拆解成"项目预算 / 工期 / 资质要求 / 评分办法 / 废标条款 / 格式要求"等结构化清单,5 分钟看懂一份招标文件。
2. **自动生成评分矩阵**:把评分办法逐项拆成 CSV 表格,每一分都对应到具体的应答章节和证据材料,杜绝漏答。
3. **按分点撰写技术方案**:严格对照评分矩阵逐项应答,每一段话都对应一个得分点。
4. **合规终审**:在交付前自动核查是否漏答、是否触碰废标条款、格式是否符合招标文件要求。
5. **沉淀公司话术库**:在 `references/phrase_library.md` 中积累高质量话术片段,越用越顺手。

---

## 三、使用方式(分三档,按你手头的 AI 工具选)
